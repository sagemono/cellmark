...
void write_log(const char *reason)
{
    ...
    if (current_mode == MODE_MEMTEST) {
        ...
        for (i = 0; i < num_spes_active; i++) {
            fprintf(fp, "  SPE%d: pass=%u bytes=%llu MB errors=%u\n", i, spe_results[i].memtest_pass, (unsigned long long)(spe_results[i].memtest_bytes / (1024*1024)), spe_results[i].memtest_errors);
            if (spe_results[i].memtest_errors > 0) {
                fprintf(fp, "    first err @+0x%08X exp:0x%08X got:0x%08X bit:%u\n", spe_results[i].memtest_err_addr, spe_results[i].memtest_err_exp, spe_results[i].memtest_err_got, spe_results[i].memtest_err_bit);
            }
        }

        if (spe_results[0].bw_read_mbps > 0) {
            fprintf(fp, "BW sync R:%u W:%u MB/s | pipe R:%u W:%u MB/s\n", spe_results[0].bw_read_mbps, spe_results[0].bw_write_mbps, spe_results[0].bw_pipe_read_mbps, spe_results[0].bw_pipe_write_mbps);
