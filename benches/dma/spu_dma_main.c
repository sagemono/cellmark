...
extern uint64_t spu_dma_put_kernel(uint64_t ea_base, uint32_t window_size, uint32_t iterations, uint32_t queue_depth, uint32_t chunk_size);
...
int main(uint64_t arg_ea, uint64_t arg2, uint64_t arg3, uint64_t arg4)
{
    ...
    if (ls_params.direction == DMA_DIR_PUT) {
        bytes = spu_dma_put_kernel(ls_params.ea_test_region, ls_params.window_size, ls_params.iterations, ls_params.queue_depth, ls_params.chunk_size);
    } else {
        bytes = spu_dma_get_kernel(ls_params.ea_test_region, ls_params.window_size, ls_params.iterations, ls_params.queue_depth, ls_params.chunk_size);
    }
