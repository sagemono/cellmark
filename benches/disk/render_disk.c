...
void render_disk_probe_view(double elapsed)
{
    ...
    cellDbgFontConsolePrintf(dbg_console, "Disk I/O Probe Suite | 32MB | %02d:%02d:%02d | %s\n\n", hours, mins, secs_i, sysmon_get_status_string());
...
void render_disk_stats(double elapsed)
{
    ...
    cellDbgFontConsoleClear(dbg_console);
    cellDbgFontConsolePrintf(dbg_console, CELLMARK_BANNER "\n");
    cellDbgFontConsolePrintf(dbg_console, "Disk I/O | %02d:%02d:%02d | 32MB file | %s\n\n", hours, mins, secs_i, sysmon_get_status_string());
